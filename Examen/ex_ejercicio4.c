...
int main (){
	...
	char nombre[20];
	char buscar [20];
	//Vairables agregadas para el examen 
	char retirar [20];
	char respuesta[3];
	int resultado2 = 0;
	...
	for (int i = 0; i < productos; i++){
		...
		for (int i = 0; i < productos; i++){
			printf("¿Quieres retirar unides de algún producto? (si/no): ");
			scanf("%s",respuesta);
			resultado2 = strcmp(respuesta, comparacion);//para comparar cadenas de caracteres se necesita usar una variable int para que mediante el uso de strcmp(cadena1 (a comparar), cadena2 (a comparar)) que se obtiene de la libreria <string.h>, si son iguales dan como resultado en int = 0 si son distintas son = 1
			if (resultado2 == 0){
			printf("Ingrese el nombre del producto a buscar: ");
			scanf("%s",retirar);
			resultado1 = strcmp(retirar, tienda[i].nombre);
		}
		}
	}
return EXIT_SUCCESS;
